...
# GIF 图片路径
GIF_PATH = r"E:\index\日常模型\盖亚爷爷桌宠\myGIF.gif"
...
class DesktopPet:
    def __init__(self, root):
        ...
        # ===== 检查 WorkBuddy 连接 =====
        if WORKBUDDY_MODE:
            self._check_workbuddy_connection()
    ...
    def animate_gif(self):
        """播放 GIF 动画帧（慢速：原速0.3倍）"""
        self.current_frame_index = (self.current_frame_index + 1) % len(self.frames)
        self.pet_img = ImageTk.PhotoImage(self.frames[self.current_frame_index])
