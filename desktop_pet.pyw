...
class DesktopPet:
    ...
    def _show_spirit_selector(self):
        ...
        selector.title("选择精灵")
        selector.geometry("260x200")
        selector.resizable(False, False)
        selector.attributes("-topmost", True)
        selector.configure(bg="#2d2d2d")
        
        # 预加载所有精灵的缩略图
        spirit_thumbnails = {}  # {spirit_key: photo_image}
        spirit_names = list(SPIRITS_CONFIG.keys())  # ["1.盖亚", "2.谱尼", "3.混元天尊", ...]
        
        for spirit_key in spirit_names:
            gif_path = get_spirit_gif_path(spirit_key)
            if gif_path and os.path.exists(gif_path):
                try:
                    thumb = Image.open(gif_path).resize((60, 60), Image.Resampling.LANCZOS)
                    spirit_thumbnails[spirit_key] = ImageTk.PhotoImage(thumb)
                except:
                    pass
        ...
        # 预览区域
        preview_frame = tk.Frame(selector, bg="#1e1e2e", width=220, height=100)
        preview_frame.pack(padx=20)
        ...
        # 上一个精灵
        top_frame = tk.Frame(preview_frame, bg="#1a1a2e")
        top_frame.place(relx=0.5, rely=0.2, anchor=tk.CENTER, width=200, height=30)
        top_name_label = tk.Label(top_frame, text="", font=("Microsoft YaHei", 9), bg="#1a1a2e", fg="#666666")
        top_name_label.pack(pady=5)
        
        # 当前精灵（可点击选中）
        center_frame = tk.Frame(preview_frame, bg="#3d3d5c")
        center_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER, width=200, height=40)
        
        def on_select():
            center_key = spirit_names[scroll_index[0]]
            self._select_spirit(center_key, selector)
        
        center_name_label = tk.Label(center_frame, text="", font=("Microsoft YaHei", 14, "bold"), bg="#3d3d5c", fg="white")
        center_name_label.pack(side=tk.LEFT, padx=15)
        
        select_btn = tk.Button(center_frame, text="✓", font=("Arial", 12), command=on_select,
                               bg="#00d4aa", fg="white", relief=tk.FLAT, cursor="hand2", width=3)
        select_btn.pack(side=tk.RIGHT, padx=10)
        
        # 下一个精灵
        bottom_frame = tk.Frame(preview_frame, bg="#1a1a2e")
        bottom_frame.place(relx=0.5, rely=0.8, anchor=tk.CENTER, width=200, height=30)
        bottom_name_label = tk.Label(bottom_frame, text="", font=("Microsoft YaHei", 9), bg="#1a1a2e", fg="#666666")
        bottom_name_label.pack(pady=5)
        
        def update_preview():
            idx = scroll_index[0]
            prev_idx = (idx - 1) % len(spirit_names)
            next_idx = (idx + 1) % len(spirit_names)
            
            # 只显示名字
            top_name_label.config(text=spirit_names[prev_idx].split(".", 1)[1])
            center_name_label.config(text=spirit_names[idx].split(".", 1)[1])
            bottom_name_label.config(text=spirit_names[next_idx].split(".", 1)[1])
        
        # 箭头按钮
        arrow_frame = tk.Frame(selector, bg="#2d2d2d")
        arrow_frame.pack(pady=10)
        
        tk.Button(arrow_frame, text="◀", font=("Arial", 10), command=scroll_up,
                  bg="#3d3d5c", fg="white", relief=tk.FLAT, cursor="hand2", width=5).pack(side=tk.LEFT, padx=10)
        tk.Button(arrow_frame, text="▶", font=("Arial", 10), command=scroll_down,
                  bg="#3d3d5c", fg="white", relief=tk.FLAT, cursor="hand2", width=5).pack(side=tk.LEFT, padx=10)
        
        # 关闭按钮
        tk.Button(selector, text="关闭", command=selector.destroy,
